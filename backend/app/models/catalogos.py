...
class Vehiculo(Base):
    ...
    # Transportista asociado a este vehículo (por placas)
    transportista_id: Mapped[int | None] = mapped_column(
        ForeignKey("cat_transportistas.id"), nullable=True, index=True
    )

    creado_en: Mapped["DateTime"] = mapped_column(DateTime(timezone=True), server_default=func.now())
    actualizado_en: Mapped["DateTime"] = mapped_column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now())

    transportista = relationship("Transportista", backref="vehiculos")
