...
def generate_packing_ogl(nave: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        for posic in orders:
            ...
            peso_caja_val = 3.8
            if cp and cp.peso_caja: peso_caja_val = float(cp.peso_caja)
            elif posic.cj_kg:
            ...
            total_pallet_val = float(posic.total_pallet or len(confirmaciones) or 1)
            peso_bruto_pallet = (p_bruto_total / total_pallet_val) if total_pallet_val > 0 else 0
        ...
        import traceback
        error_msg = f"Error generando Packing List: {str(e)}"
