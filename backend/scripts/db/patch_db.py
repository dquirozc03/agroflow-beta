    ...
    query = text("""
        ...
        BEGIN 
            IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                           WHERE table_name='clientes_ie' AND column_name='direccion_notify') THEN
                ALTER TABLE clientes_ie ADD COLUMN direccion_notify VARCHAR(1000);
                RAISE NOTICE 'Columna direccion_notify agregada correctamente.';
            ELSE
                RAISE NOTICE 'La columna direccion_notify ya existe.';
            END IF;
        END $$;
    """)
