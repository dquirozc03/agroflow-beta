...
export function PesosMedidasModal({ isOpen, onClose, registroId, onSuccess }: PesosMedidasModalProps) {
  ...
  // Pesos para cálculo (Cambiados a string | number para mejor UX al borrar 💎)
  const [taraContenedor, setTaraContenedor] = useState<number | string>(0);
  const [pesoBrutoProducto, setPesoBrutoProducto] = useState<number | string>(0);
  ...
  useEffect(() => {
    const tCont = Number(taraContenedor) || 0;
    const pProd = Number(pesoBrutoProducto) || 0;
    setPesoBrutoTotal(taraTracto + taraCarreta + tCont + pProd);
            ...
            {carretaEjes === 2 && (
              <div className="bg-amber-50/50 border border-amber-100 rounded-3xl p-5 flex items-center justify-between animate-in fade-in zoom-in-95 duration-300">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-[10px] font-black text-amber-800 uppercase tracking-widest leading-none mb-1">Configuración Se2</p>
